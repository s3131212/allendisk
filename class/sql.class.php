...
class MySQL {
        ...
        if (is_array($data)) {
            foreach ($data as $key => $val) {
                if (!is_array($data[$key])) {
                    $data[$key] = htmlspecialchars($this
                            ->databaseLink
                            ->real_escape_string($data[$key]));
                }
            }
        } else {
            $data = htmlspecialchars($this
                    ->databaseLink
    ...
    private function connect($persistant = false) {
        ...
        if ($persistant) {
            $this
                ->databaseLink = new mysqli('p:' . $this
                        ->hostname, $this
                        ->username, $this
                        ->password, $this->database);
            } else {
                $this
                    ->databaseLink = new mysqli($this
                        ->hostname, $this
                        ->username, $this
                        ->password, $this->database);
            }
            ...
            ->connect_error !== null) {
            $this
                ->lastError = 'Could not connect to server: ' . mysqli_error($this->databaseLink);
            return false;
        }
