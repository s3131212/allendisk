...
function linkcheck($type, $id, $name, $secret) {
    ...
    if (preg_match("/image\/(.*)/i", $type) || preg_match("/audio\/(.*)/i", $type) || preg_match("/video\/(.*)/i", $type) || preg_match("/text\/(.*)/i",
        $type) || $type == "application/pdf" || $type == "application/x-shockwave-flash") {
...
echo '<div class="row">';
    ...
    if ($dircheck[0]["id"] != null) {
        ...
        <div class="col-lg-3 col-md-4 col-sm-6">
            <div class="panel" data-id="<?php
echo $d['id'];?>" data-type="dir" data-download-url="home.php?dir=<?php
echo $d['id'];?>">
                <div>
...
echo $d['id'];?>" class="btn btn-default">開啟</a>
                        <a href="ajax_recycle_dir.php" data-dir-delete-id="<?php
echo $d['id'];?>" class="btn btn-danger">刪除</a>
                        <a href="ajax_rename_dir.php" data-dir-rename-id="<?php
echo $d['id'];?>" class="btn btn-cos">重新命名</a>
                        <a href="ajax_move_dir.php" data-dir-move-id="<?php
echo $d['id'];?>" class="btn btn-success">移動</a>
                        <?php
@sharedir($d['share'], $d["id"]);?>
                            <a href="ajax_set_color" data-dir-color-id="<?php
echo $d['id'];?>" class="btn btn-primary">標記</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    if ($filecheck[0]["id"] != null) {
        foreach ($db->select("file", [
            ...
            <div class="col-lg-3 col-md-4 col-sm-6">
                <div class="panel" data-id="<?php
echo $d['id'];?>" data-type="file" data-download-url="downfile.php?id=<?php
echo $d['id'];?>&download=true">
...
echo $d['id'];?>" class="btn btn-primary">重新命名</a>
                                <a href="ajax_move.php" data-move-id="<?php
echo $d['id'];?>" class="btn btn-success">移動</a>
                                <?php
@previewcheck($d['type'], $d['id']);?>
                                    <a href="ajax_set_color" data-color-id="<?php
echo $d['id'];?>" class="btn btn-cos">標記</a>
                    </div>
                </div>
            </div>
            <?php
