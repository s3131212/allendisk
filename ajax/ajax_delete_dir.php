...
function delete_dir($id) {
    ...
    foreach ($GLOBALS['db']
        ...
        $result = $GLOBALS['db']->delete('file', [
            'id' => $k["id"]
        ]);
        $result = @unlink("file/" . $k["realname"] . ".data");
    }

    $result = $GLOBALS['db']->delete('dir', [
        'id' => $id
    ]);
    return $result;
}
...
$res = $GLOBALS['db']->select('dir', ['id' => $_GET["id"]]);

if (isset($_SESSION["login"]) && isset($_SESSION["username"]) && $_SESSION["login"] && $_SESSION["username"] == $res[0]["owner"]) {
