...
$successMsg = ( isset( $_GET['s'] ) && $_GET['s'] == "1" ) ?
              "<div class=\"alert alert-success\">設定修改完成。</div>" :
              "";
?>
<!DOCTYPE html>
<html>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>啟用註冊功能</td>
                            <td>
                                <input type="checkbox" <?php if($config[ "reg"]){echo "checked";} ?> name="reg" id="reg" value="true" /></td>
                            <td>允許使用者註冊帳號，個人用網路硬碟請勿勾選</td>
                        </tr>
                        <tr>
                            <td>顯示「為何選用XXX」</td>
                            <td>
                                <input type="checkbox" <?php if($config[ "why"]){echo "checked";} ?> name="why" id="why" value="true" /></td>
                            <td>內容請至 why.php 修改</td>
                        </tr>
                        <tr>
                            <td>顯示「使用條款」</td>
                            <td>
                                <input type="checkbox" <?php if($config[ "tos"]){echo "checked";} ?> name="tos" id="tos" value="true" /></td>
                            <td>內容請至 tos.php 修改</td>
                        </tr>
                        <tr>
                            <td>管理員密碼</td>
                            <td>
                                <input type="text" value="<?php $res=$db->select(" setting ",array("name "=>"admin ")); echo $res[0]["value "]?>" name="admin" id="admin" class="form-control" />
                            </td>
                            <td>到此介面的密碼</td>
                        </tr>
                    </tbody>
