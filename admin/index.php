...
if( $newest_version ) {
    $updateText = ( version_compare( $newest_version , $current_version, '>' ) ) ?
                  "<div class=\"alert alert-warning\">新版 Allen Disk 已經發表。請至 <a href=\"http://ad.allenchou.cc\" target=\"_blank\">Allen Disk 官網</a> 下載新版！</div>" : "";
} else {
...
if( $_SESSION["alogin"] ){
    ...
    /* ================================================== */
    $text["h1 .container .text-center"] =
                                              "{$config['sitetitle']} 管理介面";

    /* ================================================== */
    $text["#1 p .container .row .panel .panel-default .col-md-4 .col-md-offset-1 .panel-body"] =
                                              "檔案數：<span class='num'>{$filecount[0]['count']} 個</span>";
    $temp = sizecount( ( $used[0]["sum"] / 1000 / 1000) );
    $text["#2 p .container .row .panel .panel-default .col-md-4 .col-md-offset-1 .panel-body"] =
                                              "佔用空間：<span class='num'>{$temp}</span>";
    $text["#3 p .container .row .panel .panel-default .col-md-4 .col-md-offset-1 .panel-body"] =
                                              "公開分享檔案數：<span class='num'>{$sharecount[0]['count']} 個</span>";

    /* ================================================== */
    $text["#1 p .container .row .panel .panel-default .col-md-4 .col-md-offset-2 .panel-body"] =
                                              "用戶數：<span class='num'>{$usercount[0]['count']} 位</span>";
    $temp = ( $config['total'] != 0 ) ? sizecount( $usercount[0]['count'] * $config['total'] / 1000 / 1000 ) : '無限制';
    $text["#2 p .container .row .panel .panel-default .col-md-4 .col-md-offset-2 .panel-body"] =
                                              "總可用空間：<span class='num'>{$temp}</span>";
}
